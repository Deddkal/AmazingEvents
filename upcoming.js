...
datosFuturos= datos.filter( element => element.date > data.currentDate)
...
function nuevaTarjeta (info){
  ...
  if (info.length == 0){
    conteiner_cards.innerHTML = `<h2 class="text-bg-danger p-3"> 
    There are no results for your search</h2>`
  }else{
...
nuevaTarjeta (datosFuturos);
$form.addEventListener("submit", (e) => e.preventDefault() )
$form.addEventListener("keyup", (e) =>{
  searchBar = $form[7].value
  datosfiltrado = filtrarSearchBar (filtradoCheck(datosFuturos, checked), searchBar)
